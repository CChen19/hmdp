...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
    ...
    public Result sendCode(String phone, HttpSession session) {
        ...
        Boolean allowed = stringRedisTemplate.opsForValue()
                .setIfAbsent(LOGIN_CODE_SEND_LIMIT_KEY + phone, "1",
                        LOGIN_CODE_SEND_LIMIT_TTL, TimeUnit.SECONDS);
    ...
    public Result login(LoginFormDTO loginForm, HttpSession session) {
        ...
        String failKey = LOGIN_CODE_FAIL_KEY + phone;
        String failCountStr = stringRedisTemplate.opsForValue().get(failKey);
        if (failCountStr != null) {
            try {
                if (Integer.parseInt(failCountStr) >= LOGIN_CODE_FAIL_MAX) {
                    return Result.fail("尝试次数过多，请稍后再试");
                }
            } catch (NumberFormatException ignored) {
                // treat corrupt counter as zero
            }
        }
